    ...
    return (
        ...
        >
            ...
            <Swiper
                ...
                dotStyle={[localStyles.dotStyle, { backgroundColor: Colors[colorScheme]?.grayScale1 }]}
                paginationStyle={localStyles.paginationStyle}>

                <OnboardingItem
                    item={OnBoardingData.onBoardingItem1}
                />
                <OnboardingItem
                    item={OnBoardingData.onBoardingItem2}
                />
                <OnboardingItem
                    item={OnBoardingData.onBoardingItem3}
                />
            </Swiper>
        </HSafeAreaView>
    );
...
const OnboardingItem = ({ item }) => {
    ...
    return (
        <View
            style={[
                localStyles.onBoardingContainer,
            ]}>
            {item?.id != 3 && <TouchableOpacity onPress={onPressSkip}><HText style={[styles.selfEnd]} type={'M16'} align={'center'} color={Colors[colorScheme]?.subText}>
                {'skip'}
            </HText></TouchableOpacity>}
            <Image source={item.image} style={localStyles.OnboardingImage} />
            <View style={localStyles.headerContainer}>
                <HText type={'S28'} align={'center'}>
                    {item.title}
                </HText>
                <HText type={'R14'} align={'center'} style={styles.mt40} color={Colors[colorScheme]?.subText}>
                    {item.desc}
                </HText>
            </View>
